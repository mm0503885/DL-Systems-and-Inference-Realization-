...
data_dir = 'food11re'
image_datasets = {x: dataset.Food11Dataset(os.path.join(data_dir, x), is_train=True, 
                                          input_transform=dataset.input_transform())
                  for x in ['skewed_training','validation']}
dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=64,
                                             shuffle=True, num_workers=0)
              for x in ['skewed_training','validation']}

dataset_sizes = {x: len(image_datasets[x]) for x in ['skewed_training','validation']}
        ...
        for phase in ['skewed_training', 'validation']:
            ...
            if phase == 'validation':
                ...
                if epoch_acc > best_acc:
                    best_acc = epoch_acc
                    best_model_wts = copy.deepcopy(model.state_dict())
                    torch.save(model.state_dict(), 'lab4_model.pht')
                    j = 0
                else:
                    j += 1
                    if j == patient:
